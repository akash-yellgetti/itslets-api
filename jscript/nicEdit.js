...
var nicEditorConfig = bkClass.extend({
    ...
    iconsPath: baseURL + '/jscript/nicEditorIcons.gif',
...
var nicEditorInstance = bkClass.extend({
    ...
    pasteClean: function () {
        ...
        /* Replace opening anchor tags with target new added */
        newSnippet = newSnippet.replace(/<a(\s+|>)/g, "<a target='_blank'$1");
        ...
        /* Replace italic tags with em */
        newSnippet = newSnippet.replace(/<pre(\s+|>)/g, "<div$1");
        /* Replace closing italic tags with closing em */
        newSnippet = newSnippet.replace(/<\/pre(\s+|>)/g, "</div$1");
...
var nicEditorAdvancedButton = nicEditorButton.extend({init: function () {
        ...
        for (itm in C) {
            ...
            var A = C[itm].type;
            if (A == "title") {
                new bkElement("div").setContent(D.txt).setStyle({fontSize: "14px", fontWeight: "bold", padding: "0px", margin: "2px 0 10px 0"}).appendTo(this.form)
            } else {
                ...
                switch (A) {
                    case"text":
                        this.inputs[itm] = new bkElement("input").setAttributes({id: itm, value: F, type: "text"}).setStyle({margin: "2px 0 10px 0", fontSize: "13px", "float": "left", height: "20px", border: "1px solid #ccc", overflow: "hidden"}).setStyle(D.style).appendTo(B);
                        break;
                    case"select":
                        this.inputs[itm] = new bkElement("select").setAttributes({id: itm}).setStyle({border: "1px solid #ccc", "float": "left", margin: "2px 0 10px 0", width: "163px"}).appendTo(B);
        ...
        new bkElement("input").setAttributes({type: "submit"}).setStyle({backgroundColor: "#efefef", border: "1px solid #ccc", margin: "3px 0 3px 60px", "float": "left", clear: "both"}).appendTo(this.form);
...
var nicEditorFontSizeSelect = nicEditorSelect.extend({sel: {1: "1&nbsp;(8pt)", 2: "2&nbsp;(10pt)", 3: "3&nbsp;(12pt)", 4: "4&nbsp;(14pt)", 5: "5&nbsp;(16pt)", 6: "6&nbsp;(18pt)", 7: "6&nbsp;(24pt)"}, init: function () {
        ...
        this.addForm(
                {
                    ...
                    target: {type: "select", txt: "Open In", style: {width: "150px"}, options: {_blank: "New Window", _self: "Current Window"}}
                },
        this.ln)
    }, submit: function (C) {
...
var nicUploadButton = nicEditorAdvancedButton.extend({nicURI: baseURL+"/save.niceimg.php", errorText: "Failed to upload image", addPane: function () {
